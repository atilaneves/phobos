 ...
 * unittest blocks for std.experimental.testing itself run as usual, but the other
 * tests are not run since std.experimental.testing runs those.
 */
private bool moduleUnitTester()
{
    foreach(module_; ModuleInfo) {
        if(module_ && module_.unitTest &&
           module_.name.startsWith("std.experimental.testing.")) {
                module_.unitTest()();
        }
    }
