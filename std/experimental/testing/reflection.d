...
unittest
{
    ...
    {
        return elements.
            map!(a => "std.experimental.testing.tests.module_with_tests." ~ a).
            array;
    }

    const expected = addModPrefix(["unittest0", "unittest1", "myUnitTestBoo"]);

    {
        import std.experimental.testing.tests.module_with_tests;
        const actual = moduleTestData!(std.experimental.testing.tests.module_with_tests).
            map!(a => a.name).
            array;
    ...
    {
        const actual = allTestData!("std.experimental.testing.tests.module_with_tests").
            map!(a => a.name).
            array;
        assertEqual(actual, expected);
    }
